...
@section('content')
                ...
                <div class="flex flex-col gap-2 ">
                    @foreach ($cart as $value)
                        <div class="flex flex-row border-zinc-300 border-2 rounded-md relative">
                            <div class="w-60 overflow-hidden border-r-2 border-zinc-300 rounded-r-md">
                                <img class="lg:w-60 w-full scale-105"
                                    src="{{ asset('upload/product/' . $value->getproduct->img) }}" alt="image">
                            </div>
                            <div class="flex flex-col p-3 w-4/5 leading-3 bg-zinc-50 relative lg:h-auto h-40">
                                <div class="w-80 leading-3">
                                    ...
                                    <p class="text-sm uppercase text-zinc-600 py-1.5"><span
                                            class="font-bold capitalize text-black">category :
                                        </span>{{ $value->getproduct->getcategory->name }}</p>
                                    <p class="text-2xl absolute bottom-12 left-2">Rs. {{ $value->getproduct->price }}</p>
                                </div>
                                <div class="flex flex-row justify-between  ">
                                    <div class="flex flex-row items-center justify-start gap-2">
                                        <p
                                            class="flex justify-center items-center hover:text-red-500 duration-300 px-2 py-1 absolute top-2  right-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                                fill="currentColor" class="bi bi-heart text-sm" viewBox="0 0 16 16">
                                    ...
                                    <div class="flex flex-row justify-between gap-2 absolute bottom-1 right-2 left-2">
                                        <div class="flex">
                                            <button name="{{ sha1(1) . $value->id }}" onclick="handlePlusQuantity(this)"
                                                class="bg-zinc-700 hover:bg-zinc-900 p-2 flex justify-center items-center duration-300 text-white rounded-md">
                                            ...
                                            <span id="{{ sha1(1) . $value->id }}"
                                                class="px-2 items-center flex justify-center"
                                                name="quantity">{{ $value->quantity }}</span>
                                            <button name="{{ sha1(1) . $value->id }}" onclick="handleMinusQuantity(this)"
                                                class="bg-zinc-700 hover:bg-zinc-900 p-2 flex justify-center items-center duration-300 text-white rounded-md">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                    fill="white" class="bi bi-dash" viewBox="0 0 16 16">
                                ...
                                <div
                                    ...
                                    <div class="text-xs px-2 absolute flex text-white top-2 justify-between w-full ">
                                        ...
                                        <div class="flex gap-2">
                                            <p class="border-red-500 text-red-500 border-2 px-3 py-0.5 rounded bg-white">
                                                30%</p>
            ...
            <div class="lg:w-1/4 w-full ps-1 pt-[56px]">
                ...
                <div class="flex flex-col bg-zinc-50 p-3 border-2 border-zinc-300 rounded-md">
                    <div class="flex flex-col">
                        ...
                        <h5 class="flex justify-between item-cneter">Delivery Fee: <span>---</span> <b>&#8377;
                                {{ $delivery_charge }}</b></h5>
                        <hr class="my-2 ">
                        <h3 class="flex justify-between item-cneter">Order Total: <span>---</span> <b>&#8377;
                                <span id="NetAmount">{{ $total + $delivery_charge }}</span></b></h3>
                    </div>
                ...
                <div class="flex flex-row justify-start font-bold text-sm my-2 ">
                    <h5 class="font-bold uppercase tracking-wider text-lg border-red-500 border-b-2 inline">Your like Items
                    </h5>
                </div>
    ...
    <script>
        const handleMinusQuantity = (params) => {
            let quantity = document.getElementById(params.name)

            if (quantity.innerText > 1) {
                $.post("{{ route('ajax_cart_update') }}", {
                        "_token": "{{ csrf_token() }}",
                        "data": params.name,
                        "quantity": "minus"
                    },
                    function(data) {
                        ...
                        totalPrice.innerText = parseInt(totalPrice.innerText) - parseInt(data.get_product.price)
                        NetAmount.innerText = parseInt(totalPrice.innerText) + {{ intval($delivery_charge)}}
                        // console.log(data);
                    }
                );
            } else {
                alert("You have to choose atleast 1 item.")
            }

        }


        const handlePlusQuantity = (params) => {
            let quantity = document.getElementById(params.name)

            if (quantity.innerText < 20) {
                $.post("{{ route('ajax_cart_update') }}", {
                        "_token": "{{ csrf_token() }}",
                        "data": params.name,
                        "quantity": "plus"
                ...
                alert("You can to choose upto 20 item.")
            }
        }
    </script>
